...
from precompute import compute_exit_decision, compute_q_matrix, compute_exit_decision_adj
...
class HeterogenousFirmsModelClass(EconModelClass):
    ...
    def allocate(self): # required
        ...
        par.m_grid = np.linspace(par.m_min,par.m_max,par.Nm)
        ...
        sol.exit_policy = np.zeros((par.Nz, par.Nm, par.Nk))
        sol.exit_policy_adj = np.zeros((par.Nz, par.Nm, par.Nk))

        sol.q = np.zeros((par.Nz, par.Nb, par.Nk))

        sol.inaction = np.zeros((par.Nz, par.Nm, par.Nk))
        sol.div_policy = np.zeros((par.Nz, par.Nm, par.Nk))

        sol.b_policy = np.zeros((par.Nz, par.Nm, par.Nk))
        sol.k_policy = np.zeros((par.Nz, par.Nm, par.Nk))
        sol.V = np.zeros((par.Nz, par.Nm, par.Nk))
