...
def _simulate(b0, k0, iz0, k_policy, b_policy, delta, alpha, xi, cf, psi, r, P, z_grid, b_grid, k_grid, T = 100):
    ...
    return np.array([b, k, div, ex, z, inv, adj_cost, iz])

def simulate(k_policy, b_policy, delta, alpha, xi, cf, psi, r, P, z_grid, b_grid, k_grid, T = 1000, N = 1):

    sims = np.zeros((N, 8, T))
    for i in range(N):
        k0 = k_grid[20]
        b0 = 0
        iz0 = 2 #np.random.choice(N_z)
        sims[i] = _simulate(b0, k0, iz0, k_policy, b_policy, delta, alpha, xi, cf, psi, r, P, z_grid, b_grid, k_grid, T = T)

    avg  = np.mean(sims, axis=0)
    condition = sims[:, 3, :] == 0  # Shape: (1000, 10000)
    # Mask the `sims` array based on the condition
    # Use `np.where` to mask only where the condition holds
    masked_sims = np.where(condition[:, None, :], sims, np.nan)  # Broadcast condition
    avg_svg = np.nanmean(masked_sims, axis=0) #average conditional on surviving

    return sims, avg, avg_svg