...
@njit 
def bellman_invest(b_next, k_next, b, k, iz, alpha, delta, psi, xi, r, cf, z_grid, b_grid, k_grid, exit, W):
    z = z_grid[iz]

    coh = z * k**alpha + (1-delta) * k - b 
    ...
    div = coh - adj_cost - k_next + b_next * q - cf
...
@njit 
def bellman_inaction(b_next, b, k, iz, alpha, delta, r, cf, z_grid, b_grid, k_grid, exit, W):
    z = z_grid[iz]
    k_next = (1-delta) * k
...
@njit
def grid_search_invest(b, k, iz, k_max, alpha, delta, psi, xi, nu, r, cf, b_min, z_grid, b_grid, k_grid, W, Nb_choice = 100, Nk_choice = 100):
    ...
    for k_next in k_choice:
        ...
        for b_next in b_choice:
            V = bellman_invest(b_next, k_next, b, k, iz, alpha, delta, psi, xi, r, cf, z_grid, b_grid, k_grid, exit,  W)
            if V > Vmax:
                Vmax = V
                b_max = b_next
                k_max = k_next
...
@njit
def grid_search_inaction(b, k, iz, alpha, delta, nu, r, cf, b_min, z_grid, b_grid, k_grid, W, Nb_choice = 100):
        ...
        for ik in range(N_k):
            for ib in range(N_b):
                ...
                if exit[iz,ib, ik]:
                    ...
                    Vinv, b_inv, k_inv = grid_search_invest(b, k, iz, k_max, alpha, delta, psi, xi, nu, r, cf, b_min, z_grid, b_grid, k_grid, W)
                    Vina, b_ina = grid_search_inaction(b, k, iz, alpha, delta, nu, r, cf, b_min, z_grid, b_grid, k_grid, W)
